...
function CompanyAnalysis() {
  ...
  const handlePeriodChange = (direction: "next" | "prev") => {
    if (direction === "next" && currentPeriodIndex > 0) {
      setActivePeriod(periods[currentPeriodIndex - 1]);
    } else if (
      direction === "prev" &&
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-900 to-slate-800 text-white p-8">
      ...
      <div className="max-w-7xl mx-auto mb-12 text-center">
        <h1 className="text-4xl md:text-5xl font-bold mb-6 bg-gradient-to-r from-cyan-400 to-teal-400 text-transparent bg-clip-text">
          Analyze companies with tons of datapoints updated in real times
        </h1>
        <p className="text-slate-300 text-lg md:text-xl">
          Stay ahead with comprehensive financial insights and real-time market
          data
        </p>
      </div>

      {/* Main Analysis Grid */}
      <div className="max-w-7xl mx-auto bg-slate-800/50 rounded-xl p-6 backdrop-blur-sm border border-slate-700">
        {/* Period Selector */}
